...
for i in $(seq $lambda_min $delta_lambda $lambda_max)
	do
		...
		sed '67,120 s/${num_conf}/'${num_conf}'/g' intermediate_file_01_${COUNTER}.txt > intermediate_file_02_${COUNTER}.txt
		sed '67,120 s/${COUNTER}/'${COUNTER}'/g' intermediate_file_02_${COUNTER}.txt > sge_qdot_02_collect_ergy_job_run_${COUNTER}.sh
